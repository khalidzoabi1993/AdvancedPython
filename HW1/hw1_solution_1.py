...
def q3_check_emails(users):
    """
    BUG: רק אימיילים תקינים נאספים, אימיילים לא תקינים פשוט נזרקים.
    האוטוגריידר מצפה שגם invalid_emails יכיל את כל הכתובות הלא תקינות.
    """
    valid_emails = []
    invalid_emails = []
    ...
    for u in users:
        ...
        if is_valid_email(email):
            ...
            # BUG – לא אוספים את האימיילים הלא תקינים לרשימה
            # invalid_emails.append(email)
            pass

    valid_emails = sorted(set(valid_emails))
    invalid_emails = sorted(set(invalid_emails))  # תישאר ריקה
