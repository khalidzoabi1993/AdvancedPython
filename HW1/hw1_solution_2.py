...
def is_valid_name(name):
    """
    BUG: בדיקה חלקית בלבד.
    - בודק שהשם לא ריק, ושהאות הראשונה גדולה,
      אבל לא בודק שאין מספרים/תווים מיוחדים ושאורך מינימלי 3.
    """
    ...
    if len(name) == 0:
        return False
    # רק בודק שהאות הראשונה גדולה
    if not name[0].isupper():
        return False
    # כל השאר לא נבדק – גם "Ab1", "A@" וכו' יעברו
    return True
